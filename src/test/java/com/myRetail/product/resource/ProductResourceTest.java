...
class ProductResourceTest {
    ...
    void setUp() {
        mockMvc = MockMvcBuilders
                ...
                .addPlaceholderValue("service.contextPath", "/product-api/v1/products")
    ...
    public void shouldBeAbleToGetProductDetails() throws Exception {
        ...
        mockMvc.perform(get("/product-api/v1/products/13860428"))
    ...
    public void shouldThrowBadRequestForInvalidInput() throws Exception {
        mockMvc.perform(get("/product-api/v1/products/ssddd"))
    ...
    public void shouldBeAbleToUpdateProductDetails() throws Exception {
        ...
        mockMvc.perform(put("/product-api/v1/products/13860428")
    ...
    public void shouldThrowBadRequestForInvalidProductIdOnUpdate() throws Exception {
        mockMvc.perform(put("/product-api/v1/products/ssddd")
                .header("Content-type","application/json")
                .content("{\"price\":\"14.99\",\"currency\":\"USD\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(content().string("[{\"errorCode\":\"API_ERR_INVALID_PRODUCTID\"" +
    ...
    @Test
    public void shouldHandleApplicationException() throws Exception {
        BigInteger id = new BigInteger("13860428");
        doThrow(new ApplicationException("Application exception has occurred")).when(productService).getProduct(id);
        mockMvc.perform(get("/product-api/v1/products/13860428"))
                .andExpect(status().isInternalServerError())
                .andExpect(content().string("[{\"errorCode\":\"500 INTERNAL_SERVER_ERROR\"," +
                        "\"errorMessage\":\"Application exception has occurred\"}]"));
    }

    @Test
    public void shouldHandleRuntimeException() throws Exception {
        BigInteger id = new BigInteger("13860428");
        doThrow(new RuntimeException("Runtime exception has occurred")).when(productService).getProduct(id);
        mockMvc.perform(get("/product-api/v1/products/13860428"))
                .andExpect(status().isInternalServerError())
                .andExpect(content().string("[{\"errorCode\":\"API_ERR_GENERIC_ERROR\"," +
                        "\"errorMessage\":\"Oops! An exception has occurred.\"}]"));
    }
}